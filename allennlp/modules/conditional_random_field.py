...
class ConditionalRandomField(torch.nn.Module):
    ...
    def _joint_likelihood(self,
        ...
        last_input_score = last_input_score.squeeze(1)                    # (batch_size,)
    ...
    def viterbi_tags(self, logits: Variable, mask: Variable) -> List[List[int]]:
        ...
        for prediction, prediction_mask in zip(logits, mask):
            ...
            tag_sequence[1:(int(sequence_length) + 1), :num_tags] = prediction[:int(sequence_length)]
            # And at the last timestep we must have the END_TAG
            tag_sequence[int(sequence_length + 1), int(end_tag)] = 0.

            # We pass the tags and the transitions to ``viterbi_decode``.
            viterbi_path, _ = util.viterbi_decode(tag_sequence[:(int(sequence_length) + 2)], transitions)
