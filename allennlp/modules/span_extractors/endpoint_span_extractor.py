...
class EndpointSpanExtractor(SpanExtractor):
    ...
    def forward(self,
        ...
        if not self._use_exclusive_start_indices:
            # (batch_size, span1_index, span2_index, ... spann_index, embedding_size)
            start_embeddings = util.batched_index_select(contextualized_embedding, span_starts)
            end_embeddings = util.batched_index_select(contextualized_embedding, span_ends)
            ...
            start_embeddings = util.batched_index_select(contextualized_embedding, exclusive_span_starts)
            end_embeddings = util.batched_index_select(contextualized_embedding, span_ends)
