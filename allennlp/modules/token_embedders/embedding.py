...
class Embedding(TokenEmbedder):
    ...
    如果想emded'TextField'，可以直接使用TextFieldEmbedder
    ...
    projection_dim : int, (optional, default=None)
        ...
        sense if ``trainable`` is ``False``.(只有在trainable=Falese才有意义)
...
def _read_pretrained_word2vec_format_embedding_file(embeddings_filename: str, # pylint: disable=invalid-name
    ...
    if embeddings_filename.endswith('.pkl'):
        with open(embeddings_filename, 'rb') as embeddings_file:
            words, pretrained_embeddings = pickle.load(embeddings_file, encoding='latin1')
            for i, word in enumerate(words):
                if word in words_to_keep:
                   embeddings[word] = pretrained_embeddings[i]
    elif embeddings_filename.endswith('.gz'):
        with gzip.open(embeddings_filename, 'rb') as embeddings_file:
            for line in embeddings_file:
                ...
                if len(fields) - 1 != embedding_dim:
                    ...
                    logger.warning("Found line with wrong number of dimensions (expected %d, was %d)",
