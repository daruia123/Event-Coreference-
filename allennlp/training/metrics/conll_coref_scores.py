...
class ConllCorefScores(Metric):
    ...
    @overrides
    def get_metric(self, reset: bool = False, all: bool = False):
        metrics = (lambda e: e.get_precision(), lambda e: e.get_recall(), lambda e: e.get_f1())

        result = {}
        values = tuple(metric(self.scorers[0]) for metric in metrics)
        result["muc"] = values
        values = tuple(metric(self.scorers[1]) for metric in metrics)
        result["b_cubed"] = values
        values = tuple(metric(self.scorers[2]) for metric in metrics)
        result["ceafe"] = values
        avg = tuple(sum(metric(e) for e in self.scorers) / len(self.scorers)
