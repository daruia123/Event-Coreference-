...
from allennlp.data.dataset_readers.dataset_utils.utils import (
        bio_tags_to_spans,
        TypedStringSpan
)
...
class SpanBasedF1Measure(Metric):
    ...
    def __init__(self,
        ...
        if label_encoding:
            if label_encoding not in ["BIO"]:
                raise ConfigurationError("Unknown label encoding - expected 'BIO'.")
        elif tags_to_spans_function is None:
            raise ConfigurationError(
                'At least one of the (label_encoding, tags_to_spans_function) should be provided.'
            )
        self._label_encoding = label_encoding
        self._tag_to_spans_function = tags_to_spans_function
    ...
    def __call__(self,
             ...
             prediciton_map: Optional[torch.Tensor] = None):
        """
        Parameters
        ----------
        ...
        predictions, gold_labels, mask, prediciton_map = self.unwrap_to_tensors(predictions, gold_labels,
                                                                            mask, prediciton_map)

        num_classes = predictions.size(-1) #int
        if (gold_labels >= num_classes).any():
            raise ConfigurationError("A gold label passed to SpanBasedF1Measure contains an "
                                 "id >= {}, the number of classes.".format(num_classes))

        sequence_lengths = get_lengths_from_binary_sequence_mask(mask)
        # max(-1)返回了数字和索引，在取1就是只要索引
        # (batch_size, num_classes)
        argmax_predictions = predictions.max(-1)[1]

        if prediciton_map is not None:
            argmax_predictions = torch.gather(prediciton_map, 1, argmax_predictions)
            gold_labels = torch.gather(prediciton_map, 1, gold_labels.long())
