...
def test(model: Model,
    ...
    iterator = data_iterator(instances, num_epochs=1, cuda_device=cuda_device, for_training=False, shuffle=False)
    logger.info("Iterating over dataset")
    generator_tqdm = Tqdm.tqdm(iterator, total=data_iterator.get_num_batches(instances))
    results = []
    for batch in generator_tqdm:
# check!
        result = model.decode(model(**batch))
        result = {"clusters": result.get("clusters", [])}
        results.append(sanitize(result))
        metrics = model.get_metrics()
        description = ', '.join(["%s: %.2f" % (name, value) for name, value in metrics.items()]) + " ||"
        generator_tqdm.set_description(description, refresh=False)

    return results, model.get_metrics(reset=True)
...
def evaluate_from_args(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    logger.info("Reading test data from %s", evaluation_data_path)
    instances = dataset_reader.read(evaluation_data_path)

    iterator = BasicIterator(batch_size=1)
    iterator.index_with(model.vocab)

    results, metrics = test(model, instances, iterator, args.cuda_device)
