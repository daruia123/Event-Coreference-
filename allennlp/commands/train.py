...
class Train(Subcommand):
    def add_subparser(self, name: str, parser: argparse._SubParsersAction) -> argparse.ArgumentParser:
        ...
        subparser = parser.add_parser(name, description=description, help='Train a model') #子命令
...
def train_model_from_args(args: argparse.Namespace):
    ...
    """
    train_model_from_file(args.param_path,  # Training_config/coref_chinese.json
                          args.serialization_dir, # debug
                          args.overrides,  #''
                          args.file_friendly_logging, # False
                          args.recover)  # False, fun = <function train_model_from_args...>,include_package=[]
...
def train_model_from_file(parameter_filename: str,
    ...
    ----------
    param_path : ``str``
        A json parameter file specifying an AllenNLP experiment.    # 指定AllenNLP实验的json参数文件。
    serialization_dir : ``str``
        The directory in which to save results and logs. We just pass this along to   # 保存结果和日志
        :func:`train_model`.
    overrides : ``str``
...
def create_serialization_dir(params: Params, serialization_dir: str, recover: bool) -> None:
    ...
    if os.path.exists(serialization_dir):
        if serialization_dir == '/output':
            ...
            return
        if not recover:
            logger.warning(f"Serialization directory ({serialization_dir}) already exists.")
...
def train_model(params: Params,
    ...
    # deepcopy()复制了一个新的对象，改变原来的object不影响它
    serialization_params = deepcopy(params).as_dict(quiet=True)

    # CONFIG_NAME = "config.json"
    with open(os.path.join(serialization_dir, CONFIG_NAME), "w") as param_file:
        json.dump(serialization_params, param_file, indent=4)  # indent表示输出格式
